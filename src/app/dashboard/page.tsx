...
function ExamSwitcher({ plans, activePlan, onSelect, streak, score, remaining, correctAnswers, answeredQuestions, studyMinutes }: { plans: OnboardingState[]; activePlan: OnboardingState; onSelect: (id: ExamId) => void; streak: number; score: number; remaining: number | null; correctAnswers: number; answeredQuestions: number; studyMinutes: number }) {
  const [open, setOpen] = useState(false);
  const [actionExamId, setActionExamId] = useState<ExamId | null>(null);
  const [detailOpen, setDetailOpen] = useState<"rank" | "streak" | "score" | "date" | null>(null);
  ...
  const toggleDetail = (detail: "rank" | "streak" | "score" | "date") => {
  ...
  return <div className="relative flex items-center gap-1 sm:gap-2">
    ...
    <button type="button" onClick={() => toggleDetail("rank")} aria-label={`${exam.name} ranking`} aria-expanded={detailOpen === "rank"} title={`${exam.name} ranking`} className={`hidden h-11 w-11 place-items-center rounded-xl transition sm:grid ${detailOpen === "rank" ? "bg-[#fff4d6] ring-2 ring-[#e9b04d]" : "text-[#d68a00] hover:bg-[#fff8e7]"}`}><svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="2"><path d="M8 4h8v4a4 4 0 0 1-8 0V4Z" strokeLinejoin="round"/><path d="M8 6H4v1a4 4 0 0 0 4 4M16 6h4v1a4 4 0 0 1-4 4M12 12v4M8 20h8M9 16h6" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
    ...
    {detailOpen ? <>
      ...
      <section role="dialog" aria-label={`${exam.name} ${detailOpen} details`} className={`absolute right-0 top-14 z-40 rounded-3xl border-2 border-[var(--line)] bg-white p-5 shadow-[0_20px_60px_rgba(21,32,50,0.18)] ${detailOpen === "rank" ? "w-[min(24rem,calc(100vw-2rem))]" : "w-80"}`}>
        {detailOpen === "rank" ? <ExamRankCard exam={exam} score={score} studyMinutes={studyMinutes} activeDays={streak} compact/> : detailOpen === "score" ? <>
...
function ExamWorkspace({ plan, records }: { plan: OnboardingState; records: ReviewRecord[] }) {
  const exam = getExam(plan.examId);
  if (!exam) return null;
  return <section className="w-full" aria-labelledby={`exam-${exam.id}`}><h2 id={`exam-${exam.id}`} className="sr-only">{exam.name} dashboard</h2><SubjectPerformance plan={plan} records={records}/></section>;
}
